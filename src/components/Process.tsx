  ...
  const steps = [
    {
      num: "01",
      title: "Reach Out",
      desc: "Call (561) 834-5705 for a free 15-minute consultation. We'll talk through fit, answer your questions, and schedule your first appointment if it feels right.",
    },
    {
      num: "02",
          ...
          {steps.map((step, idx) => (
            <div key={idx} className="flex flex-col text-left">
              ...
              <div className="bg-white p-6 rounded-2xl shadow-sm border border-[#F0EDE8] flex-grow">
                <h3 className="font-playfair text-[20px] md:text-[22px] font-bold text-[#25323D] mb-3">{step.title}</h3>
                <p className="font-lato text-[16px] md:text-[18px] text-[#25323D] leading-relaxed">{step.desc}</p>
              </div>
            </div>
          ))}
        </div>
